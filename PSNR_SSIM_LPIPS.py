...
def parse_args():
    ...
    parser.add_argument('--img_dir', type=str,
                        default='/home/damon/Downloads/DASR-main/remote_sensing/Data/AID/HR',
                        help='image directory')
    parser.add_argument('--scale', type=str, default='4',
                        help='super resolution scale')
    parser.add_argument('--resume', type=int, default=726,
...
def main():
    ...
    # path sr images
    SR_dir = '/home/damon/Downloads/DASR-main/remote_sensing/Data/AID/bin/matlab_lr/LR_4'
    ...
    SR_coll = io.ImageCollection(SR_dir + '/*.png')
    ...
    for i in range(len(coll)):
        ...
        # inference
        sr = pil_image.open(SR_coll.files[i]).convert('RGB')
        sr = np.array(sr).astype(np.uint8)
        hr = np.array(hr).astype(np.uint8)

        psnr = calculate_psnr(sr, hr, 4)
        ssim = calculate_ssim(sr, hr, 4)
        eval_psnr += psnr
        eval_ssim += ssim
        ...
        # with open("MHAN_WHU_x4.txt", "a") as f:

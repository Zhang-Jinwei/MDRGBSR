...
class BatchBlur(nn.Module):
    ...
    def forward(self, input, kernel):
        ...
        if len(kernel.size()) == 2:
            ...
            # a = kernel.squeeze(0).squeeze(0).cpu().numpy()
            # a = a[0, 0, :]
...
# def SRM(hr, scale, sig=2.6):
#     gen_kernel = Gaussin_Kernel( kernel_size=21, sig=sig)
#     blur = BatchBlur(kernel_size=21)
#     hr = np.array(hr).astype(np.uint8)
#     H, W, C = hr.shape
#     b_kernels = gen_kernel(1,0)  # B degradations
#
#     # blur
#     hr_blured = blur(hr.view(1, -1, H, W), b_kernels)
#     hr_blured = hr_blured.view(-1, C, H, W)  # BN, C, H, W
#     lr_blured = bicubic(hr_blured, scale=1/scale)
#     lr_blured = torch.clamp(lr_blured.round(), 0, 255)
#     return lr_blured.view(1, C, H // int(scale), W // int(scale))
#
# if __name__ == '__main__':
#     import PIL.Image as pil_image
#     hr = pil_image.open('/home/damon/Downloads/DASR-main/remote_sensing/Data/WHU-RS19/HR/bridge_18.jpg').convert('RGB')

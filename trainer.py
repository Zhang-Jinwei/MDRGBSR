...
class Trainer():
    ...
    def train(self):
        ...
        if epoch <= self.args.epochs_encoder:
            ...
            # if (epoch - self.args.epochs_encoder) % 75 == 0:
            #     self.args.lr_decay_sr += 1
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr

        self.ckp.write_log('[Epoch {}]\tLearning rate: {:.2e}'.format(epoch, Decimal(lr)))
        # with open("model_add_X2_.txt", "a") as f:
        #     f.write('lr:' + str(lr) + ' \t')
        self.loss.start_log()
        self.model.train()
        ...
        for batch, (hr, _, idx_scale) in enumerate(self.loader_train):
            ...
            if epoch <= self.args.epochs_encoder:
                if (batch + 1) % self.args.print_every == 0:
                    ...
                    with open("model_4_MSR_.txt", "a") as f:
                        f.write(' Epoch: \t' +str(epoch) +
                                'Loss [contrastive loss: '+str(losses_contrast.avg)+'\t\n')
            else:
                if (batch + 1) % self.args.print_every == 0:
                    self.ckp.write_log(
                        'Epoch: [{:04d}][{:04d}/{:04d}]\t'
                    ...
                    with open("model_4_MSR_.txt", "a") as f:
                        f.write(' Epoch: \t' + str(epoch) +
                                ' Loss [SR loss:'+str(losses_sr.avg)+' contrastive loss: '+str(losses_contrast.avg)+'\t\n')
                    # with open("train_AID_x4_SRLOSS.txt", "a") as f:
                    #     f.write(str(losses_sr.avg)+'\t\n')
                    # with open("train_AID_x8_CLOSS.txt", "a") as f:
                    #     f.write(str(losses_contrast.avg)+'\t\n')
        ...
        torch.save(
            model_dict,
            os.path.join(self.ckp.dir, 'model', 'model_add_{}.pt'.format(epoch)) ###
        )
    ...
    def test(self):
        ...
        self.model.eval()
        # self.model.load_state_dict(torch.load('/home/damon/Downloads/DASR-main/experiment/blindsr_x4_bicubic_iso/model/model_600.pt'), strict=False)
        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (hr, filename, _) in enumerate(self.loader_test):
                    ...
                    # with open("test_WHU_Viaduct.txt", "a") as f:
                    #     f.write('PSNR: '+str(psnr)+'\t'+'SSIM: '+str(ssim)+'\t\n')
                    # save results
                    if self.args.save_results:
                        save_list = [sr]
                        # filename = filename[0]
                        ###
                        filename = filename[0] + '_CBM'                         ###
                        self.ckp.save_results(filename, save_list, scale)       ###
                        save_list = [lr.squeeze(0)]
                        ###
                ...
                with open("test_add.txt", "a") as f:
                    f.write('ALL:\n'+'PSNR: ' + str(eval_psnr / len(self.loader_test)) + '\t' + 'SSIM: ' + str(eval_ssim / len(self.loader_test)) + '\t\n')
